...
import (
	...
	"github.com/Tixcash-foundation/go-opera/utils/signers/internaltx"
)
...
func (s *PrivateAccountAPI) ListAccounts() []common.Address {
	...
	for _, account := range s.am.Accounts() {

		addresses = append(addresses, string(account[:]))
	}
	return addresses*/
	return s.am.Accounts()
}
...
func (s *PublicBlockChainAPI) GetBalance(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (*hexutil.Big, error) {
	...
	//return (*hexutil.Big)(&big.Int{}), state.Error()
}
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f TXH) exceeds the configured cap (%.2f TXH)", feeFloat, cap)
	}
	return nil
}
